...
if ($option -eq 39) {
    ...
    # Define the .dll file and the registry path
...
# Check if the file is removed and also remove the entry from the registry
if (-Not (Test-Path $filePath)) {
    Write-Output "$filePath removed successfully."
    
    # Get all entries from the registry path
    $registryEntries = Get-ItemProperty -Path $registryPath

    # Loop through each entry and remove the one that matches the .dll file
...
# Remove any .dll files that are not present in the screenshot
# Assuming you have an array of the allowed .dlls from the screenshot
...
# Get all .dll entries from the registry path
$registryEntries = Get-ItemProperty -Path $registryPath
...
if ($option -eq 40) {
    ...
    try {
        ...
        $directories = $userInput.Split(",").Trim()  # Split the input into an array and trim spaces
        ...
        # Path to Sysinternals sigcheck.exe
        ...
        foreach ($directory in $directories) {
            if (Test-Path $directory) {
                foreach ($extension in $fileExtensions) {
                    try {
                        # Get all files of the current extension in the current directory
                    ...
                    foreach ($file in $files) {
                        ...
                        try {
                            # Use sigcheck for a detailed check (optional)
                        ...
                        # Check if the file is not signed or signature is not valid
                        if ($signature.Status -ne "Valid" -or $sigcheckResult -match "not signed") {
                            ...
                            Write-Host $outputMessage  # Output to the user screen
        
                            # Optional: Output signature and sigcheck details for debugging
                            $signature | Out-File -Append -FilePath $outputFile
                            $sigcheckResult | Out-File -Append -FilePath $outputFile
        
                            # Optional: Output to the user screen for debugging
